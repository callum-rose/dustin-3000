...
public:
  ...
  MotorThread (String name, int pwmPin, int directionPin, int encoderAPin, int encoderBPin, int encoderPulsesPerRev, float gearRatio) : Thread()
  {
    ...
    m_pid = new PID(&m_inputSpeed, &m_outputPwm, &m_setpointSpeed, 0.6, 5, 0.3, DIRECT);
...
void setup() 
{
  ...
  sonar1Thread.setInterval(333); // in ms
  sonar2Thread.setInterval(333);

  btCheckThread.setInterval(500);
